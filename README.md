...
All source code used to generate the results, figures and some data(depending on size) in the paper can be accessed as soon as the paper is published. The calculations and figure generation are all run in Python 3.10. Results generated by the code are saved in `Paper_figures`.
