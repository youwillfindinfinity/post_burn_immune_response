    ...
    for interval_idx, (start_time, end_time) in enumerate(time_intervals):
        ...
        for cell_idx, cell_type in enumerate(cell_count_labels):
            ...
            for exp_idx in range(n_exp):
                ...
                if exp_idx == 0: # experiment 4 is control
...
# cytokine_mean_diff_boxplot('combi_code/datafiles/', 8, exp = "diff_boxplot", show="on", outputdir = "Output/")
...
def cell_count_mean_diff_boxplot(file, n_exp, exp, show, outputdir):
    ...
    # colors = cm.rainbow(np.linspace(0, 1, len(time_intervals)))

    # Create a figure with subplots, 2 rows and 2 columns
    fig, axs = plt.subplots(2, 2, figsize=(12, 12))

    for interval_idx, (start_time, end_time) in enumerate(time_intervals):
        all_data = []  # Initialize a list to hold the data for box plots
        
        for cell_idx, cell_type in enumerate(cell_count_labels):
            data_for_cell_type = []  # Data for the current cell type
            
            for exp_idx in range(0, n_exp):
                path = "scan_iteration_{}/{}cellcount.txt".format(exp_idx, file)
                data = pd.read_csv(path, sep=",")  # Load the data
                ...
                # Get the cell count data for the current time interval, experiment, and cell type
                cell_count_data = interval_data[cols[cell_idx]]  # Access the correct column
                data_for_cell_type.extend(cell_count_data)
            
            all_data.append(data_for_cell_type)
        
        # Calculate subplot position
        row = interval_idx // 2
        col = interval_idx % 2
        
        # Create the box plot for the current time interval
        box = axs[row, col].boxplot(all_data, patch_artist=True)
        for patch, color in zip(box['boxes'], colors):
            patch.set_facecolor(color)
        axs[row, col].set_title(f"Box Plot - {start_time}-{end_time} hours")
        axs[row, col].set_xticklabels(cell_count_labels, rotation=45, ha='right')
        ...
        if col == 0:
            axs[row, col].set_ylabel('Cell Count')
    ...
    # Save the figure
    fig.savefig("{}cell_count_box_plot.png".format(outputdir), dpi=300)
...
cell_count_mean_diff_boxplot('combi_code/datafiles/', 8, exp = None, show="on", outputdir = "Output/")
